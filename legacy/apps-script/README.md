# Brainule v1 — Apps Script study-review app

> **Status: legacy.** This is the original Brainule, superseded by Brainule 2.0
> (the Node.js/TypeScript tutor platform at the repository root — see
> [`../../README.md`](../../README.md)). It is kept here because it is still
> deployable and still backs the live Apps Script project. It receives no new
> features; everything below continues to work as written, from this directory.
...
```
legacy/apps-script/
├── .clasp.json               — clasp config: links this directory to the Apps Script project
├── README.md                 — this file
├── preview.py                — builds src/ into a single local preview.html
...
All clasp commands must be run from this directory — `.clasp.json` lives here,
not at the repository root.

```bash
git clone <repo-url>
cd Brainule/legacy/apps-script
clasp push --force
```
...
```bash
# 0. cd legacy/apps-script  (all clasp commands run from here)
# 1. Edit files in src/
...
`preview.py` assembles `src/` into a single `preview.html` with mock bootstrap data
and a stubbed `google.script.run`, then serves it:

```bash
cd legacy/apps-script
python3 preview.py          # builds + serves at http://localhost:8080
python3 preview.py --build  # writes preview.html only, no server
```

---

## Data collection (superseded)

In Brainule 2.0 this is handled server-side by the Node platform; the notes below
describe the v1 design only.
